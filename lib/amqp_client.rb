...
class Chef
  module IndexQueue
    class AmqpClient
      ...
      def amqp_client
        unless @amqp_client
          begin
            ...
            Mixlib::Authorization::Log.debug "Starting AMQP connection with client settings: #{@amqp_client.inspect}"
            @amqp_client.start
            @amqp_client.qos(:prefetch_count => 1)
          rescue Bunny::ServerDownError => e
            Mixlib::Authorization::Log.fatal "Could not connect to rabbitmq. Is it running, reachable, and configured correctly?"
            raise e
          rescue Bunny::ProtocolError => e
            Mixlib::Authorization::Log.fatal "Connection to rabbitmq refused. Check your rabbitmq configuration and chef's amqp* settings"
            raise e
          end
        end
        @amqp_client
      ...
      def disconnected!
        Mixlib::Authorization::Log.error("Disconnected from the AMQP Broker (RabbitMQ)")
        @amqp_client = nil
        reset!
      end
      ...
      def queue_for_object(obj_id)
        ...
        begin
          ...
          disconnected!
          if (retries += 1) < 2
            Mixlib::Authorization::Log.info("Attempting to reconnect to the AMQP broker")
            retry
          else
            Mixlib::Authorization::Log.fatal("Could not re-connect to the AMQP broker, giving up")
            raise
          end
        end
      end

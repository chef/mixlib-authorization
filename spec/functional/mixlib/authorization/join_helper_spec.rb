...
describe JoinHelper do
  # !!!: Uncomment when you fix this file. <NPK 2012-01-26>
  # before(:all) do
  ...
  xit "adds a join type to the mixed in class" do
    @extended_class.join_type(:AJoinType)
    @extended_class.join_type_for_class.should == :AJoinType
  end

  xit "adds join properties to the mixed in class" do
  ...
  describe "when extending an actor model" do
    ...
    describe "when the model does not have an auth join" do
      xit "creates an auth join and an Actor join" do
      ...
      xit "raises an authorization error when attempting to update a join" do
      ...
      xit "does not cause a ruckus when attempting to delete the join object" do
        lambda { @model.delete_join }.should_not raise_error
      end

      xit "raises an error that === ArgumentError when attempting to fetch a join" do
        lambda { @model.fetch_join }.should raise_error(ArgumentError, "Cannot find join for ExampleActorBasedModel #{@model.id}")
      end

      xit "raises an error that === ArgumentError when attempting to fetch a join ACL" do
        lambda { @model.fetch_join_acl }.should raise_error(ArgumentError, "Cannot find join for ExampleActorBasedModel #{@model.id}")
      end

      xit "raises an error that === ArgumentError when attempting to determine if an actor is authorized for an operation" do
      ...
      xit "raises an errot that === ArgumentError when attempting to update a join ACE" do
    ...
    describe "when the model has a join" do
      ...
      xit "updates an auth join object with updated Auth data" do
      ...
      xit "deletes a join object but not the actor-type-specific auth join object" do
        @model.delete_join
      ...
      xit "fetches the auth side data" do
        @model.fetch_join.should == {"id" => @join_doc["auth_object_id"] }
      end

      xit "fetches a join acl" do
      ...
      xit "queries the auth object to determine if an actor is authorized for an action" do
        auth_object = @model.fetch_auth_join_for(@join_doc)
